#!/usr/bin/env python3
"""Run depth-7 GPU beam search on a single dataset.

Usage: python run_depth7.py <DatasetName> <gpu_id>
Example: python run_depth7.py EEG 0
...
# Import GPU primitives — strip the hardcoded CUDA device
...
# GPU ops (CuPy equivalents of the numpy SB/SU)
GSB = {
...
def gpu_beam(X_gpu, y_gpu, max_depth=7):
    N, d = X_gpu.shape
    ...
    for depth in range(2, max_depth + 1):
        ...
        for opn, opf in VBIN:
            try:
                ...
                for bi in range(B):
                    for fi in range(d):
                        new.append((float(f1_all[bi * d + fi]), beam[bi][1].b(opn, fi)))
        ...
        for opn, opf in VUN:
            try:
                ...
                for bi in range(B):
                    new.append((float(f1u[bi]), beam[bi][1].u(opn)))
            except Exception:
                pass
        ...
        log.info("  d%d: best=%.4f K=%d", depth, beam[0][0], len(new))
...
for fold_i, (tr, te) in enumerate(cv.split(X, y)):
    ...
    best_f1, best_trace = gpu_beam(X_aug_tr, y_tr, max_depth=max_d)
...
with open(f"pilot_d7_{DATASET.lower()}.json", "w") as f:
