...
def gpu_beam_search_large(X_gpu, y_gpu, feat_names, feat_idx=None, max_depth=3, beam_width=100):
...
def run_dataset(
    ...
    log.info("=" * 70)
    log.info(
        "%s (N=%d, d=%d, prev=%.2f) — %d folds, depth=%d, beam=%d",
        name,
        X.shape[0],
        X.shape[1],
        y.mean(),
    ...
    for fold_i, (tr, te) in enumerate(splits):
        ...
        for trial in range(n_subspaces):
            k = min(subspace_k, d)
            feat_idx = (
                list(range(d)) if d <= k else sorted(rng.choice(d, k, replace=False).tolist())
            )
        ...
        if (fold_i + 1) % 25 == 0:
            ...
            log.info(
                ...
                fold_i + 1,
                total,
                np.mean(astar_train_f1s[-25:]),
                np.mean(astar_f1s[-25:]),
                rate,
                (total - fold_i - 1) / rate,
            )
...
def main():
    ...
    for r in all_results:
        log.info(
            "%-12s N=%-6d d=%-2d  test=%.4f  %s  GB:%.1f%s RF:%.1f%s LR:%.1f%s  gpu:%.0fs",
            r["name"],
            r["N"],
            r["d"],
            r["test_f1"],
