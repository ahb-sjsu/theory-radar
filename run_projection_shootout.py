...
def make_projection(name, X_tr, y_tr, X_te):
    ...
    if name == "raw":
        ...
        return (np.hstack([X_tr, p_tr]), np.hstack([X_te, p_te]),
        ...
        return (np.hstack([X_tr, pca_tr, pls_tr]),
...
def run_shootout(name, X, y, n_repeats=100, n_folds=5):
    ...
    for proj_name in PROJECTIONS:
        ...
        for fold_i, (tr, te) in enumerate(splits):
            try:
                X_tr_aug, X_te_aug, aug_names = make_projection(
                    proj_name, X[tr], y[tr], X[te])
        ...
        log.info("  %-12s test=%.4f+/-%.4f  train=%.4f  gap=%.3f  %s (%.0f%%)  %.0fs",
                 proj_name,
                 f1s_test.mean(), f1s_test.std(),
                 f1s_train.mean(),
                 f1s_train.mean() - f1s_test.mean(),
                 top[0][:30], 100 * all_results[proj_name]["stability"],
                 elapsed)
    ...
    log.info("")
    log.info("  %-12s %-8s %-8s %-5s  %-30s  %-10s %-10s %-10s",
             "Projection", "Test F1", "Stab%", "Gap", "Formula", "vs GB", "vs RF", "vs LR")
    log.info("  " + "-" * 110)
    ...
    for proj_name, mr in all_results.items():
        ...
        log.info("  %-12s %.4f   %4.0f%%   %.3f  %-30s  %-10s %-10s %-10s",
                 proj_name, mr["test_mean"], 100*mr["stability"],
                 mr["gap"], mr["formula"][:30],
                 bl_str["GB"], bl_str["RF"], bl_str["LR"])
