...
def gpu_batch_auroc(vals, labels):
    N, K = vals.shape
    pos = float(labels.sum()); neg = N - pos
    if pos == 0 or neg == 0:
...
def meta_search_heuristic(X, y, feat_names, max_depth=3):
    ...
    for depth in range(2, max_depth + 1):
        new = {}
        for pn in prev:
            p = catalog[pn]; pv = p["values"]
            for j in range(d):
                for op, fn in BINARY_OPS.items():
                    cn = f"({pn} {op} {feat_names[j]})"
                    if cn in catalog or cn in new: continue
                    try:
                        cv = fn(pv, X_gpu[:, j])
                        cv = cp.nan_to_num(cv, nan=0., posinf=1e10, neginf=-1e10)
                        if float(cp.var(cv)) < 1e-20: continue
                        new[cn] = {"depth": depth, "values": cv,
            ...
            for op, fn in UNARY_OPS.items():
                ...
                try:
                    ...
                    if float(cp.var(cv)) < 1e-20: continue
                    new[cn] = {"depth": depth, "values": cv,
                               "n_features": p["n_features"], "parent": pn}
                    children[pn].append(cn)
                except: pass
        ...
        if new:
            ...
            for start in range(0, len(names), 50000):
                ...
                for i, n in enumerate(chunk):
                    ...
                    catalog[n] = new[n]
                del vm; cp.get_default_memory_pool().free_all_blocks()
        prev = list(new.keys())

    # Free values
    for n in catalog:
        if "values" in catalog[n]: del catalog[n]["values"]
    ...
    if not alive_nodes or not dead_nodes:
        return None, 0., {"n_formulas": len(catalog), "n_alive": len(alive_nodes)}
    ...
    best_crit_name, best_crit_fn, best_thresh, best_prune = None, None, 0., 0.

    for cname, cfn in CRITERIA:
        alive_vals = [cfn(v["f1"], v["auroc"], v["depth"], v["n_features"])
                      for _, v in alive_nodes]
        dead_vals = [cfn(v["f1"], v["auroc"], v["depth"], v["n_features"])
                     for _, v in dead_nodes]

        min_alive = min(alive_vals)
        pruned = sum(1 for dv in dead_vals if dv < min_alive)
        rate = pruned / len(dead_vals) if dead_vals else 0.
    ...
    log.info("  Meta: %d formulas, %d alive, %d dead", len(catalog), len(alive_nodes), len(dead_nodes))
    log.info("  Best criterion: %s < %.4f → prune %.1f%%",
             best_crit_name, best_thresh, 100*best_prune)

    return best_crit_fn, best_thresh, {
...
def astar_beam_search(X_gpu, y_gpu, feat_names, criterion_fn, criterion_thresh,
                       f1_target=0.0, max_depth=4, beam_width=100,
                       n_subspaces=10, subspace_k=6, rng=None):
    ...
    for trial in range(n_subspaces):
        k = min(subspace_k, d)
        feat_idx = list(range(d)) if d <= subspace_k else sorted(rng.choice(d, k, replace=False).tolist())
        ...
        for li in range(dk):
            ...
            beam.append((priority, f1_val, local_names[li], X_sub[:, li].copy(),
                        FormulaTrace(gi), auc_val, 1))
        ...
        for depth in range(2, max_depth + 1):
            ...
            for i in range(K):
                ...
                new_beam.append((priority, f1_val, name, vals_mat[:, i].copy(),
                                trace, auc_val, nf))
...
def run_dataset(name, X, y, features, n_repeats=200, n_folds=5,
                max_depth=4, n_subspaces=20, subspace_k=6):
    total = n_repeats * n_folds
    log.info("=" * 70)
    log.info("A* BEAM SEARCH: %s (N=%d, d=%d) — %d folds",
             name, X.shape[0], X.shape[1], total)
    log.info("  depth=%d, beam=100, subspaces=%d, k=%d",
             max_depth, n_subspaces, subspace_k)
    log.info("=" * 70)
    ...
    for fold_i, (tr, te) in enumerate(splits):
        ...
        train_f1, formula_name, trace = astar_beam_search(
            ...
            crit_fn, crit_thresh, f1_target=0.0,
            max_depth=max_depth, beam_width=100,
            n_subspaces=n_subspaces, subspace_k=subspace_k, rng=rng)
        ...
        if (fold_i + 1) % 50 == 0:
            ...
            log.info("  A* %d/%d  train=%.3f test=%.3f  %.2f/s  ETA %.0fs",
    ...
    log.info("  A* done: %.0fs (%.2f folds/s), meta-search: %.1fs avg (%d cached)",
             gpu_time, total/gpu_time, avg_meta, n_folds)
    ...
    for bn in ["GB", "RF", "LR"]:
        ...
        results[bn] = {"mean": float(bf.mean()), "std": float(bf.std()),
                        "diff": float(diff.mean()), "sigma": float(abs(t_stat)),
                        "p": float(p_val), "dir": "A*>" if diff.mean() > 0 else f"{bn}>"}
    ...
    return {"name": name, "meta_stats": meta_stats, "test_f1": float(astar_f1s.mean()),
            "train_f1": float(astar_train_f1s.mean()), "formula": top[0],
            "baselines": results, "meta_time": float(avg_meta), "search_time": gpu_time}
...
def main():
    ...
    for name, X, y, feats in datasets:
        ...
        r = run_dataset(name, X, y, feats, n_repeats=200, n_folds=5,
                        max_depth=4, n_subspaces=n_sub, subspace_k=k)
    ...
    for r in all_results:
        log.info("%-15s test=%.4f  meta_prune=%.1f%%  %s  GB:%.1f%s RF:%.1f%s LR:%.1f%s",
                 r["name"], r["test_f1"],
