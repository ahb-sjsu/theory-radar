...
SB = {"+":lambda a,b:a+b,"-":lambda a,b:a-b,"*":lambda a,b:a*b,
      "/":lambda a,b:a/(b+1e-30),"max":lambda a,b:np.maximum(a,b),
      "min":lambda a,b:np.minimum(a,b),"hypot":lambda a,b:np.sqrt(a**2+b**2),
      "diff_sq":lambda a,b:(a-b)**2,"harmonic":lambda a,b:2*a*b/(a+b+1e-30),
...
def quick_meta(X_tr, y_tr, feat_names):
    ...
    Xg = cp.asarray(X_tr,dtype=cp.float64); yg = cp.asarray(y_tr,dtype=cp.float64)
    d = Xg.shape[1]
    if d > 6:
    ...
    for pi in range(ds):
        ...
        for opn, opf in VBIN:
            try:
                ...
                for j in range(ds):
                    if float(cp.var(out[:,j])) < 1e-20: continue
                    d2_vals.append(out[:,j]); d2_meta.append({"d":2,"nf":2,"parent":pi})
            except: pass
    ...
    if d2_vals:
        ...
        for i in range(len(d2_vals)):
            cat.append({"f1":float(f1s2[i]),"d":2,"nf":d2_meta[i]["nf"],
                        "parent":d2_meta[i]["parent"]})
        del vm

    del Xg, yg; cp.get_default_memory_pool().free_all_blocks()
    for c in cat:
        if "vals" in c: del c["vals"]

    # Viability
    gb = max(c["f1"] for c in cat)
    for c in cat: c["alive"] = 1 if c["f1"] >= gb - 0.005 else 0
    # Propagate
    children = defaultdict(list)
    for i,c in enumerate(cat):
        if "parent" in c: children[c["parent"]].append(i)
    for i,c in enumerate(cat):
        if c["d"]==1 and any(cat[ci]["alive"]==1 for ci in children.get(i,[])): c["alive"]=1

    shallow = [c for c in cat if c["d"]==1]
    alive = [c for c in shallow if c["alive"]==1]
    dead = [c for c in shallow if c["alive"]==0]
    if not alive or not dead: return None, 0.

    CRIT = [
        ("f1", lambda c: c["f1"]),
        ("f1/d", lambda c: c["f1"]/max(c["d"],1)),
    ]
    best_name, best_th, best_r = "f1/d", 0., 0.
    for cn, cf in CRIT:
        av = [cf(c) for c in alive]; dv = [cf(c) for c in dead]
        ma = min(av); pr = sum(1 for x in dv if x<ma)/len(dv)
        if pr > best_r: best_r=pr; best_th=ma; best_name=cn
...
def astar_pca_beam(X_aug_gpu, y_gpu, feat_names, crit_fn, crit_th,
                    feat_idx=None, max_depth=3, beam_width=100):
    ...
    if feat_idx is not None:
        ...
        Xs = X_aug_gpu; ln = feat_names; feat_idx = list(range(d_full))
    d = Xs.shape[1]; yf = y_gpu.astype(cp.float64)
    ...
    for rank, li in enumerate(order[:B]):
        gi = feat_idx[li]; fv = float(f1s[li])
        if crit_fn and crit_fn(fv, 0.5, 1, 1) < crit_th: continue
        h = 0.0 if fv >= 0.99 else 1.0 - fv
        beam.append((1.0+h, fv, ln[li], Xs[:,li].copy(), FT(gi), 1))
        if fv > best_f1: best_f1,best_nm,best_tr = fv,ln[li],FT(gi)
    beam.sort(key=lambda x:x[0]); beam = beam[:beam_width]

    for depth in range(2, max_depth+1):
        if not beam: break
        Bc = len(beam)
        bv = cp.stack([b[3] for b in beam], axis=1)
        bv3 = bv[:,:,None]; fv3 = Xs[:,None,:]
        ...
        for on,of in VBIN:
            try:
                ...
                for bi in range(Bc):
                    for li in range(d):
                        gi=feat_idx[li]
                        meta.append((f"({beam[bi][2]} {on} {ln[li]})",
                                    beam[bi][4].b(on,gi),
        ...
        if not chunks: break
        av = cp.concatenate(chunks,axis=1); K=av.shape[1]
        ...
        nb = []
        for i in range(K):
            fv=float(f1c[i]); nm,tr,nf=meta[i]
            if crit_fn and crit_fn(fv,0.5,depth,nf)<crit_th: continue
            h = 0.0 if fv>=0.99 else 1.0-fv
            nb.append((float(depth)+h, fv, nm, av[:,i].copy(), tr, nf))
            if fv>best_f1: best_f1,best_nm,best_tr = fv,nm,tr
        nb.sort(key=lambda x:x[0]); beam = nb[:beam_width]
        del av,chunks; cp.get_default_memory_pool().free_all_blocks()
...
def run_dataset(name, X, y, feat_names, n_repeats=200, n_folds=5,
                n_pca=8, max_depth=3, beam_width=100, n_subspaces=15, subspace_k=10):
    total = n_repeats * n_folds
    d = X.shape[1]
    log.info("=" * 70)
    log.info("FULL PIPELINE: %s (N=%d, d=%d) — %d folds, %d PCs, depth=%d, %d subspaces",
             name, X.shape[0], d, total, n_pca, max_depth, n_subspaces)
    log.info("=" * 70)
    ...
    for fold_i, (tr, te) in enumerate(splits):
        k = min(n_pca, d)
        pca_obj = PCA(n_components=k)
        pca_tr = pca_obj.fit_transform(X[tr])
        pca_te = pca_obj.transform(X[te])
        ...
        best_f1, best_nm, best_tr_obj = 0., "", None
        ...
        for trial in range(n_subspaces):
            ...
            f1, nm, tr_obj = astar_pca_beam(X_gpu, y_gpu, aug_names, cf, ct,
                                             feat_idx=fi, max_depth=max_depth,
                                             beam_width=beam_width)
            if f1 > best_f1: best_f1, best_nm, best_tr_obj = f1, nm, tr_obj
        ...
        # Fair test eval
        vtr = best_tr_obj.ev(X_aug_tr); vte = best_tr_obj.ev(X_aug_te)
    ...
    return {"name":name, "test_f1":float(af.mean()), "train_f1":float(at.mean()),
            "formula":top[0], "stability":float(top[1]/len(formulas)), "baselines":res}
...
def main():
    ...
    except Exception as e: log.warning("Diabetes: %s",e)
    ...
    try:
        ...
        yr = bn.target.astype(int); y=(yr==yr.max()).astype(int)
        datasets.append(("Banknote", X, y, [f"b{i}" for i in range(X.shape[1])]))
    except Exception as e: log.warning("Banknote: %s",e)
    ...
    except Exception as e: log.warning("Ionosphere: %s",e)
    ...
    log.info("%-15s %-8s %-8s %-30s %-10s %-10s %-10s",
             "Dataset","Train","Test","Formula","vs GB","vs RF","vs LR")
    log.info("-"*100)
    for r in all_res:
        log.info("%-15s %.4f  %.4f  %-30s %6.1f%s %s  %6.1f%s %s  %6.1f%s %s",
                 r["name"], r["train_f1"], r["test_f1"], r["formula"][:30],
                 r["baselines"]["GB"]["sigma"],"s",r["baselines"]["GB"]["dir"],
                 r["baselines"]["RF"]["sigma"],"s",r["baselines"]["RF"]["dir"],
                 r["baselines"]["LR"]["sigma"],"s",r["baselines"]["LR"]["dir"])
