...
def gpu_beam_search_subspace(X_gpu, y_gpu, feat_names, feat_idx,
    ...
    for depth in range(2, max_depth + 1):
        ...
        for nc in name_chunks: all_names.extend(nc)
        for tc in trace_chunks: all_traces.extend(tc)
...
def fuzzed_search(X_gpu, y_gpu, feat_names, n_trials=50,
                   subspace_k=6, max_depth=4, beam_width=50, rng=None):
    ...
    for trial in range(n_trials):
        ...
        f1, name, trace = gpu_beam_search_subspace(
            X_gpu, y_gpu, feat_names, feat_idx,
...
def run_dataset(name, X, y, features, n_repeats=200, n_folds=5,
                n_trials=50, subspace_k=6, max_depth=4):
    total = n_repeats * n_folds
    log.info("=" * 70)
    log.info("%s (N=%d, d=%d) — %d folds, %d trials/fold, k=%d, depth=%d",
             name, X.shape[0], X.shape[1], total, n_trials, subspace_k, max_depth)
    log.info("=" * 70)
    ...
    for fold_i, (tr, te) in enumerate(splits):
        ...
        train_f1, formula_name, trace = fuzzed_search(
            ...
            n_trials=n_trials, subspace_k=subspace_k,
            max_depth=max_depth, beam_width=50, rng=rng)
        ...
        if (fold_i + 1) % 50 == 0:
            ...
            log.info("  Fuzz %d/%d  train=%.3f test=%.3f  %.2f folds/s  ETA %.0fs",
    ...
    for bname in ["GB", "RF", "LR"]:
        ...
        log.info("  vs %-3s: %.4f  diff=%+.4f  %.1f%s  %s",
                 bname, b["mean"], b["diff"], b["sigma"], "s", b["dir"])

    return {
        "name": name, "n_trials": n_trials, "subspace_k": subspace_k,
        "max_depth": max_depth,
...
def main():
    ...
    for name, X, y, features in datasets:
        ...
        r = run_dataset(name, X, y, features,
                        n_repeats=200, n_folds=5,
                        n_trials=n_trials, subspace_k=6, max_depth=4)
    ...
    for r in all_results:
        log.info("%-15s test=%.4f  formula=%s  vs GB %.1f%s  vs RF %.1f%s  vs LR %.1f%s",
                 r["name"], r["test_f1"], r["formula"][:30],
