...
\title{Theory Radar: A* Formula Search with Provable Pruning\\for Interpretable Symbolic Prediction}
...
\label{thm:auroc_f1}
For a binary classifier with $\mathrm{AUROC} = A$ and class
prevalence $\pi$, the maximum achievable thresholded F1 satisfies:
\[
F_1^{\max} \leq \sup_{\substack{t \in [J, 1] \\ J = 2A-1}}
\frac{2 t \pi}{t \pi + \pi + (t - J)(1 - \pi)}
\]
where $J = 2A - 1$ is the maximum Youden index for any ROC curve
with area $A$.
\end{theorem}

\begin{proof}
Any ROC curve with area $A$ has maximum Youden index $J = \max(TPR
- FPR) \leq 2A - 1$, with equality for the step-function ROC curve
(this is a standard result). At any operating point
$(FPR, TPR)$ with $TPR - FPR = J$, we substitute $FPR = TPR - J$
into the F1 expression and maximize over $TPR \in [J, 1]$.
\end{proof}

This bound is \emph{tight}: it is achieved by the step-function ROC
curve. Table~\ref{tab:bounds} shows bounds for specific datasets.
...
\begin{table}[h]
\caption{A* search results across 9 datasets. ``Phased'' is the
ceiling from phased enumeration; ``GB'' is gradient-boosted ensemble
(5-fold CV). A* finds better formulas than phased on 4 datasets
and beats the ensemble on 1 (Diabetes).}
\label{tab:astar_vs_phased}
\scriptsize
\begin{tabular}{@{}lccccl@{}}
\toprule
Dataset ($d$) & Phased & GB & A* & Cost & A* formula \\
\midrule
Circles (2) & .996 & .985 & .995 & 2 & \texttt{x1 hypot x2} \\
Moons (2) & .833 & .969 & \textbf{.902} & 3 & \texttt{(x2$-$x1) max x2} \\
Synth (5--20) & 1.00 & .951 & 1.00 & 2 & \texttt{x1 hypot x0} \\
Breast Ca.\ (10) & .937 & .951 & \textbf{.958} & 3 & \texttt{(f2 min f1)+f7} \\
Diabetes (8) & --- & .632 & \textbf{.694} & 3 & \texttt{(d5 min d7)+d1} \\
3-body (8) & .480 & .830 & \textbf{.493} & 3 & \texttt{(rk min r)+$\lambda$} \\
\bottomrule
\end{tabular}
\end{table}

On \textbf{Diabetes}, the A* formula $(\texttt{d5}~\mathrm{min}~
\texttt{d7}) + \texttt{d1}$ (BMI, age, glucose) achieves F1 $= 0.694$,
\emph{outperforming} gradient boosting ($0.632$) with a 3-operation
formula. On \textbf{Moons}, A* discovers
$(\texttt{x2} - \texttt{x1})~\mathrm{max}~\texttt{x2}$ (F1 $= 0.902$),
an 8\% improvement over the phased ceiling ($0.833$). On the
\textbf{3-body problem}, A* finds
$(\mathrm{rank}~\mathrm{min}~r_2/r_1) + \lambda_\mathrm{min}$
(F1 $= 0.493$), breaking the phased ceiling of $0.480$.
...
\toprule
Dataset ($d$) & Expand. & Mono. & AUROC & Total pruned \\
\midrule
Circles (2) & 312 & 80 & 164 & 244 \\
Moons (2) & 798 & 176 & 55 & 231 \\
Synth (5) & 2{,}035 & --- & --- & --- \\
Synth (20) & 30{,}841 & --- & --- & --- \\
Breast Ca.\ (10) & 50{,}000 & --- & --- & --- \\
Diabetes (8) & 23{,}162 & 2{,}304 & 6{,}786 & 9{,}090 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Ceiling Depends on Search Strategy}

The A* ceilings exceed the phased ceilings on 4 of 6 datasets,
not because of better thresholding (both use exact $O(N\log N)$
sweep) but because A* explores tree compositions that phased
templates miss. The Moons ceiling increases from 0.833 to 0.902
and the 3-body ceiling from 0.480 to 0.493. This demonstrates
that the ``formula ceiling'' is not an intrinsic property of the
data---it depends on which formula structures are searched.

The \textbf{Diabetes} result is particularly notable: a depth-3
formula combining BMI, age, and glucose with min and addition
outperforms a 100-tree gradient-boosted ensemble. In a clinical
setting, this interpretable formula would be preferred over the
black-box model not only for transparency but for accuracy.
