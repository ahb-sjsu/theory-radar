...
\emph{Tier~3 (representation augmenters):} PCA, Tucker, kernel,
or neural projections that give depth-3 formulas implicit access
to all $d$ features, closing the accuracy gap with linear models
on high-dimensional datasets.

Across 17 established benchmarks (N from 208 to 130K, $d$ from 4
to 60) with fair held-out evaluation, Theory Radar identifies
datasets where a three-feature formula matches or beats gradient
boosting (Diabetes: $25\sigma$ advantage, Wine: $16\sigma$) and
datasets where ensembles genuinely win (EEG: $246\sigma$).  Both
outcomes are reported.  The Monotone Invariance Theorem enables
...
\textbf{PCA.}  Standard principal components.  Each component is
a linear combination of all $d$ features.  A formula
$\min(\mathrm{pc}_0, f_5) + \mathrm{pc}_2$ implicitly uses all
features through the PCA projections.

\textbf{Tucker decomposition.}  Decomposes the pairwise feature
interaction tensor $\mathbf{x} \otimes \mathbf{x}$ via HOSVD,
capturing how features \emph{interact}, not just individual
variance directions.

\textbf{Kernel projections.}  Random Fourier features or kernel
PCA for nonlinear manifold structure.

\textbf{Neural projections.}  A one-hidden-layer ReLU network
trained on the classification task; the hidden activations become
features.  Provides data-adaptive nonlinear projections.

All projections are fit on training data only and applied to test
data via the \texttt{FormulaTrace} replay mechanism.  On Breast
Cancer, PCA projections improved test $F_1$ from 0.955 (raw) to
0.963, closing the gap with gradient boosting (0.967) from
$21\sigma$ to $6\sigma$.
...
all $F_1$ on held-out test data.  Bold = formula wins.  The
``Formula'' column shows the most frequently discovered formula
across folds (stability ranges from 18\% to 100\%; see text).}
\label{tab:sigma}
\scriptsize
...
\midrule
Breast Ca. & $\max(f_{20},f_{27}){+}f_{23}$ & .955 & .017 & 20.9 GB & 29.0 RF & 52.1 LR \\
Wine & $\min(w_6,w_0){+}w_{12}$ & .975 & .017 & \textbf{30.8 A*} & 2.7 RF & 9.9 LR \\
Diabetes & $\min(d_5,d_7){+}d_1$ & .672 & .024 & \textbf{27.7 A*} & \textbf{29.7 A*} & \textbf{23.4 A*} \\
Banknote & $(b_0{+}b_1){+}b_2$ & .986 & .003 & 25.2 GB & 21.5 RF & \textbf{28.3 A*} \\
Ionosph. & $\sqrt{i_4^2{+}i_3^2}{-}i_2$ & .912 & .033 & 44.8 GB & 53.8 RF & 0.4 n.s. \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Formula wins (fair evaluation).}
On Diabetes, the three-feature formula $\min(d_5, d_7) + d_1$
shows a large, consistent fold-level advantage over all three
baselines (effect sizes $23$--$30\sigma$).  A clinician can
compute this formula by hand (minimum of insulin and age, plus
glucose), yet it achieves test $F_1 = 0.672$ vs.\ GB's $0.631$.
On Wine, $\min(\text{flavanoids}, \text{alcohol}) + \text{proline}$
shows a large advantage over gradient boosting ($30.8\sigma$
effect size, $F_1 = 0.975$ vs.\ $0.923$).
On Banknote, $(b_0 + b_1 + b_2)$ outperforms LR ($28.3\sigma$)
while being 100\% stable across 1000 folds.

\textbf{Baseline wins.}
On Breast Cancer ($d=30$), the formula uses only 3 of 30 features.
Logistic regression, which linearly combines all 30, wins at
$52\sigma$.  On Ionosphere ($d=34$), the formula's test $F_1$
drops to 0.912 (gap of 0.033 from train), while RF achieves
0.949.
...
costs accuracy.  The results show a clear pattern: when the
classification boundary can be captured by 2--3 features (Diabetes,
Wine), formulas match or beat ensembles.  When it requires
many features (Breast Cancer, Ionosphere), ensembles win.
The train--test gap (0.003--0.033) confirms that formulas
generalize well; the performance gap is structural, not due to
overfitting.
