...
On 9 of 31 datasets (29\%), the formula matches or beats default
...
No existing tool provides a systematic empirical comparison within a
...
\textbf{Positioning.}
Theory Radar occupies a distinct niche in this landscape: it is the
first system, to our knowledge, that performs bounded symbolic search
...
\texttt{nan\_to\_num}$(nan{=}0,\; posinf{=}10^{10},\;
neginf{=}{-}10^{10})$.

\textbf{Constant detection.}
Candidates with standard deviation $< 10^{-12}$ are assigned
$F_1 = 0$.

\textbf{PLS configuration.}
\texttt{sklearn.cross\_decomposition.PLSRegression} with
$n_{\mathrm{components}} = \min(8,\; d{-}1,\; N_{\mathrm{train}}{-}1)$.
...
win ($\Delta F_1 > 0$, CI above zero), tie (CI includes zero),
...
All nine formula wins use PLS-projected features (or raw features
on datasets with $d \leq 4$ where projection is unnecessary).
...
suggests that medical/clinical datasets often have relatively simple
...
operation, number of positive axillary nodes), the formula operates
on raw features without projection.  The survival signal is
concentrated in the node count, and a simple threshold on a
formula involving node count and age suffices.  This is the ideal
case for symbolic classification: $d$ is so low that a three-term
formula is not structurally limited, and the small $N$ does not
provide enough complex interactions to give ensembles an advantage.
...
positive axillary nodes), the formula operates on raw features
directly---no projection needed because the formula can access all
three features within its depth-3 structure.  The survival signal
is concentrated in the node count and age interaction.  This is the
ideal case for symbolic classification: the data \emph{is}
low-dimensional, the formula \emph{is} the model---there is no
indirection, no projection layer, no weight table to consult.  The
formula can be read directly as a clinical rule: survival
probability depends on the interaction of positive axillary nodes
(the dominant signal) with patient age.  This transparency is
unmatched by any ensemble method.  It is also worth noting that
Haberman is a notoriously difficult dataset with high class
imbalance (73.5\% survival); the formula's $\Delta F_1 = +0.175$
over gradient boosting is one of the largest margins in the entire
benchmark.
...
Haberman trivially achieves 1.00 (no projection is used).  This
...
On Haberman ($d = 3$), no indirection exists---the formula operates
...
The single exception is Ionosphere ($d = 34$, $N/d = 10.3$), which
improves at depth~5, suggesting that its decision boundary requires
more than three terms to approximate.  This confirms that depth~3
provides the best generalization on 29 of 31 datasets.
...
pure raw-feature formulas (like Haberman) do not require.
