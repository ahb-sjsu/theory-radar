...
def beam_search(X_gpu, y_gpu, feat_names, feat_idx, max_depth=3, beam_width=100):
    ...
    for rank, li in enumerate(order[:B]):
        ...
        beam_names.append(ln[li])
        beam_traces.append(FT(gi))
    best_f1 = float(f1s[order[0]])
    best_nm = beam_names[0]
    best_tr = beam_traces[0]
    ...
    for depth in range(2, max_depth + 1):
        ...
        for rank, i in enumerate(ti):
            beam_vals[:, rank] = av[:, i]
            beam_names.append(an[i])
            beam_traces.append(at[i])
            if float(tf[rank]) > best_f1:
                best_f1 = float(tf[rank])
                best_nm = an[i]
                best_tr = at[i]
        del av, chunks
        try:
...
def process_fold(args):
    ...
    best_f1, best_nm, best_tr = 0.0, "", None
    ...
    with _gpu_sem:
        ...
        for trial in range(n_subspaces):
            ...
            f1, nm, tr_obj = beam_search(
                X_gpu, y_gpu, aug_names, fi, max_depth=max_depth, beam_width=beam_width
            )
            if f1 > best_f1:
...
def run_dataset(
    ...
    log.info("=" * 70)
    log.info(
        "PARALLEL PIPELINE: %s (N=%d, d=%d) — %d folds, %d workers",
        name,
        X.shape[0],
        d,
        total,
    ...
    log.info(
        "  PCs=%d, depth=%d, beam=%d, subspaces=%d, k=%d",
        n_pca,
        max_depth,
        beam_width,
        n_subspaces,
    ...
    fold_args = [
        (fold_i, tr, te, X, y, feat_names, n_pca, max_depth, beam_width, n_subspaces, subspace_k)
...
def main():
    ...
    for r in all_res:
        log.info(
            "%-12s N=%-6d test=%.4f  %s  GB:%.1f%s RF:%.1f%s LR:%.1f%s",
            r["name"],
            r["N"],
            r["test_f1"],
            r["formula"][:25],
