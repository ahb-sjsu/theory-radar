...
def run_dataset_d4(name, X, y, features, n_repeats=200, n_folds=5):
    ...
    log.info("=" * 70)
    log.info("%s DEPTH 4 (N=%d, d=%d) — %d folds [FAIR EVAL]", name, X.shape[0], X.shape[1], total)
    log.info("=" * 70)
    ...
    for fold_i, (tr, te) in enumerate(splits):
        ...
        train_f1, formula_name, trace = gpu_beam_search_traced(
            X_gpu[tr_gpu], y_gpu[tr_gpu], features, max_depth=4, beam_width=100
        )  # DEPTH 4
        ...
        if (fold_i + 1) % 100 == 0:
            ...
            log.info(
                "  D4 fold %d/%d  train=%.3f  test=%.3f  %.1f/s",
                fold_i + 1,
                total,
...
def main():
    ...
    datasets.append(("BreastCancer-D4", X, bc.target, [f"f{i}" for i in range(X.shape[1])]))
    ...
    try:
        ...
        datasets.append(("Ionosphere-D4", X, y, [f"i{i}" for i in range(X.shape[1])]))
